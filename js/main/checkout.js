(function($) {
  ...
  $(document).ready(function() {
    ...
    $('#clients').owlCarousel({
      ...
      responsive: {
          0: {items: 5, margin: 20}, 
          768: {items: 11, margin: 20}, 
          1100: {items: 15, margin: 25},
      }
  });
    ...
    for (var i = 0; i < itemz.length; i++) {
      ...
      if(daAc.includes('Bank America')) { daColor = coBoa; } 
      else if(daAc.includes('Barclays')) { daColor = coBoa; } 
      else if(daAc.includes('BBVA')) { daColor = coBoa; } 
      else if (daAc.includes('Chase')) { daColor = coChase; } 
      else if (daAc.includes('Chime')) { daColor = coChime; } 
      else if (daAc.includes('Citi')) { daColor = coCiti; } 
      else if (daAc.includes('Huntington')) { daColor = coHunt; } 
      else if (daAc.includes('Navy')) { daColor = coNavy; } 
      else if (daAc.includes('Truist')) { daColor = coTruist; } 
      else if (daAc.includes('RBC')) { daColor = coRbc; } 
      else if (daAc.includes('Scotia')) { daColor = coRbc; } 
      else if (daAc.includes('TD')) { daColor = coRbc; } 
      else if (daAc.includes('PNC')) { daColor = coRbc; } 
      else if (daAc.includes('M&T')) { daColor = coRbc; } 
      else if (daAc.includes('Wells')) { daColor = coWells; } 
      else if (daAc.includes('Wood')) { daColor = coWood; } 
    ...
    window.addEventListener("load", () => {
      ...
      tooltip3.setActiveElements([{ datasetIndex: 0, index: 0 }]);
      chart3.update();

    const tooltip4 = chart4.tooltip;
      tooltip4.setActiveElements([{ datasetIndex: 0, index: 0 }]);
      chart4.update();

      const tooltip2 = chart2.tooltip;
      tooltip2.setActiveElements([{ datasetIndex: 0, index: 0 }]);
      chart2.update();
    });
  });
