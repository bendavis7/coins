...
function addToCartClick(event) {
    ...
    var accoun = button.parentElement.parentElement.children[5].children[0].innerText;
    ...
    if(localStorage.getItem('banklogs') && ((JSON.parse(localStorage.getItem('banklogs')).length) > 0)){
        ...
        for(var i = 0; i < cartItemNames.length; i++) {
            if(cartItemNames.length >= 2) {
                ...
                setTimeout(() => {
                    var shortCutFunction = 'success'; var msg = `Your cart is full... <br> checkout logs in cart. <hr class="to-hr hr15-bot">`; 
